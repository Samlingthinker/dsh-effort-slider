...
@media (prefers-reduced-motion: reduce){.fgSaaq_statusMax{animation:none}}`;
		...
		function createAppearanceStore() {
			...
			const save = async (appearance) => {
				...
				// 首次保存前若快照尚未加载（load 失败或竞态），先拉一次宿主快照，
				// 拿到当前 revision 再提交，避免 expectedRevision=undefined 被宿主拒绝（400）。
				...
				if (revision === void 0) {
					try {
						...
						// 宿主路由不可用：放弃本次保存，让设置页回滚并展示错误。
						state = { ...state, loaded: true, error: "宿主设置路由不可用" };
						publish();
						return false;
					}
				}
		...
		function SettingsSection(props) {
			...
			const choose = (id) => {
				...
				// 乐观更新按钮；失败时回滚并展示错误（save 返回 false 或 reject 均视为失败）。
				setSnapshot((prev) => ({ ...prev, appearance: id, error: void 0 }));
				setSaving(true);
				Promise.resolve(setAppearance(id)).then((ok) => {
					if (ok !== true) {
						setSnapshot((prev) => ({
							...prev,
							appearance: previous,
							error: "保存失败，请检查宿主设置路由是否可用"
						}));
					}
				}).catch((error) => {
					setSnapshot((prev) => ({
