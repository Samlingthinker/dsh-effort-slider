 ...
 * 面板配色恒定跟随官方主题（theme.getTheme().active.colorScheme），
 * 不另设外观选项——官方「外观」切浅色/深色时，本插件实时切换，
 * 避免两套外观设置的重复。
 * 面板配色取自 Harness 色板（浅/深两套），字体继承 Harness
 * （不设 font-family），与整体界面风格一致。
 *
 * 结构源自 @captain1275/dsh-client-ui-skin-aurora（npm 包声明 BSD-3-Clause；
 * 源码仓库 LICENSE 为 Apache-2.0，Copyright 2026 zhu1090093659 (linxin),
 * CAPTAIN1275）的 EffortPanel + useWebglFire（三pass 渲染管线），改造点：
 *   1. 渲染着色器改为「流光粒子」（流动光线流 + 行进粒子 + 拖尾 + 前沿光波）；
 *   2. 面板 CSS 重构为两套主题变量（浅色/深色），随官方主题切换；
 *   3. 去掉皮肤背景/视频/主题等无关部分。
 *
 * 数据流全部走官方 API 与服务：
 *   - connection.api.sessions.models / selectModel   读写推理等级
 *   - theme 服务 + theme/change 事件                   面板配色实时跟随官方主题
 */
...
`;
		...
		function useWebglFire(canvasRef, getSlider, getActive) {
			...
			(0, react.useEffect)(() => {
				...
				// prefers-reduced-motion：只渲染一帧静态流光，不启动持续动画循环。
				...
				function ensureLoop() {
					...
					// prefers-reduced-motion：渲染一帧静态流光（弹簧直接贴合目标），不启动循环。
					if (reducedMotion) {
						loopRunning = true;
				...
				function renderFrame(t) {
					...
					// 空闲判定：目标为 0 且不活跃时，等弹簧衰减到稳定（尾光自然消散）再停循环；
					// 若弹簧仍在运动（拖回 Off 的尾迹），继续渲染直至收敛。
					const settled = Math.abs(springValue) < 0.001 && Math.abs(springVelocity) < 0.001;
					if (sliderRef.current <= 0 && !activeRef.current && settled) {
		 ...
		 * 首个像素档（官方 4 档下为 Low）及以上档位时轨道变成动画像素场——
		 * 扫过式显现 + 流动闪烁；Low = 绿色，High = 蓝色（色板按 lowBlend 平滑过渡），
		 * MAX = 紫色（色板按 blend 平滑过渡）；
		 ...
		 * @param getMax - 返回当前是否处于像素档（Low / High / MAX）。
		 ...
		 * @param getLowBlend - 返回 Low→High 连续过渡系数 0..1（0 = 绿色，1 = 蓝色）。
		 */
		// 像素场色板：MAX = 紫色（整条轨道）；High = 蓝色；Low = 绿色（左半轨道）。
		// 绿色版深端锚点 #2EA86C，11 级 tone 按蓝/紫同款衰减曲线
		// 平滑过渡到浅端：色相恒定 ~150°，饱和度 70% → 24% 递减。
		...
		/** Low 档色板（官方新增档位后 low 成为首个像素档）：绿色系，
		 *  11 级 tone 按蓝/紫同款衰减曲线从深翡翠绿平滑过渡到浅端。 */
		const GREEN_PALETTE = {
			left: [214, 224, 212],
			tones: [
		...
		/** 深色主题的 off 侧锚点：暗绿灰 / 暗蓝灰 / 暗紫灰（浅色主题的灰白左端在深色下改暗）。 */
		const DARK_GREEN_LEFT = [22, 38, 28];
		const DARK_BLUE_LEFT = [26, 24, 44];
		const DARK_PURPLE_LEFT = [24, 19, 40];
		function useMaxPixelField(canvasRef, getMax, getBlend, getThemeMode, getThumb, getLowBlend) {
			const maxRef = (0, react.useRef)(false);
			maxRef.current = getMax();
			/** Low→High 连续过渡系数：0=绿色，1=蓝色（与 High→MAX 蓝→紫同构）。 */
			const lowBlendRef = (0, react.useRef)(1);
			lowBlendRef.current = getLowBlend ? getLowBlend() : 1;
			...
			(0, react.useEffect)(() => {
				...
				function draw(time) {
					...
					const lowBlend = lowBlendRef.current;
					...
					const elapsed = state.reduced ? 0 : Math.max(0, time - startedAtRef.current);
					// 色板两段式连续：Low→High 绿→蓝按 lowBlend、High→MAX 蓝→紫按 blend
					// 插值（含 tone 数组与 clamp 范围），与 High→MAX 过渡同构。
					// off 侧锚点随主题切换：深色主题用暗绿灰/暗蓝灰/暗紫灰，避免左端灰白。
					const darkTheme = themeModeRef.current === "dark";
					const greenLeft = darkTheme ? DARK_GREEN_LEFT : GREEN_PALETTE.left;
					const blueLeft = darkTheme ? DARK_BLUE_LEFT : BLUE_PALETTE.left;
					const purpleLeft = darkTheme ? DARK_PURPLE_LEFT : PURPLE_PALETTE.left;
					const mixArr = (x, y, t) => [mix(x[0], y[0], t), mix(x[1], y[1], t), mix(x[2], y[2], t)];
					const mixPalette = (a, b, t) => ({
						left: mixArr(a.left, b.left, t),
						tones: a.tones.map((tone, i) => mixArr(tone, b.tones[i], t)),
						highlight: mixArr(a.highlight, b.highlight, t),
						peak: mixArr(a.peak, b.peak, t),
					...
					const greenPal = { ...GREEN_PALETTE, left: greenLeft };
					const bluePal = { ...BLUE_PALETTE, left: blueLeft };
					const purplePal = { ...PURPLE_PALETTE, left: purpleLeft };
					// 先绿→蓝（low 段），再蓝→紫（high→max 段）。
					const pal = mixPalette(mixPalette(greenPal, bluePal, lowBlend), purplePal, blend);
					...
					// prefers-reduced-motion：冻结所有时间参数，只呈现静态帧。
					const frozenTime = state.reduced ? 0 : Math.max(0, time - startedAtRef.current);
					...
					for (const c of cells) {
						...
						let color;
						// --- Low/High 光带模型：off 侧灰白 → 深 tone 渐变（70/30），闪烁只做亮度/高光调制。 ---
				...
				function ensureLoop() {
					...
					if (!maxRef.current) return;
					// prefers-reduced-motion：只渲染一帧静态图，不启动动画循环。
					if (state.reduced) {
...
.fgSaaq_level0{color:#c882a0bf}.fgSaaq_level1{color:#58bc8abf}.fgSaaq_level2{color:#82aac8bf}.fgSaaq_level3{color:#c084fcf2;text-shadow:0 0 10px #a855f7b3}.fgSaaq_level4{color:#d8b4fe;text-shadow:0 0 12px #a855f7}
...
@media (prefers-reduced-motion: reduce){.fgSaaq_statusMax{animation:none}}`;
		...
		/** 解析实际明暗模式：官方主题快照已将 system 解析为 dark/light。 */
		function resolveThemeMode(themeSnapshot) {
			return themeSnapshot?.active?.colorScheme === "dark" ? "dark" : "light";
		}
		//#endregion
		//#region EffortPanel
			...
			(0, react.useEffect)(() => {
				...
				connection.api.sessions.models({ sessionId }).then((response) => {
					...
					if (response.result.ok) {
						...
						setDirectory(null);
						setError(response.result.error?.message ?? "模型目录加载失败");
					}
				}).catch((err) => {
					if (alive) {
						setDirectory(null);
		...
		function EffortPanel(props) {
			...
			const { directory, error: directoryError } = useDirectory(connection, sessionId);
			...
			// 依赖含 currentEffortId：面板打开期间模型/档位被外部切换时，滑块跟随回同步。
			(0, react.useLayoutEffect)(() => {
				if (directory !== null && !dragging) {
					setRawValue(initialRaw);
					setDragging(false);
				}
			}, [directory, currentEffortId]);
			...
			// 首个像素档位：官方 4 档（off/low/high/max）下为 Low（index 1），
			// 3 档（off/high/max）下为 High——Off 档(index 0)永不走像素场。
			...
			// Low → High 连续过渡：色板绿→蓝、粒子风格沿用 High 光带模型，
			// 按 smoothstep 平滑插值（与 High→MAX 蓝→紫过渡同构）。
			const lowPos = pixelStart * step100;
			const rawLowBlend = highPos > lowPos ? (slider100 - lowPos) / (highPos - lowPos) : 1;
			const clampedLowBlend = Math.min(1, Math.max(0, rawLowBlend));
			const lowBlend = clampedLowBlend * clampedLowBlend * (3 - 2 * clampedLowBlend);
			...
			useMaxPixelField(pixelRef, () => pixelActive, () => maxBlend, () => themeMode, () => slider100, () => lowBlend);
			 ...
			 *  pointerup 与 blur 都可能触发本函数（拖动松手 → pointerup，
			 *  随后焦点转移 → blur）；UI 吸附幂等可重复执行，后端写入用
			 *  时间窗去重，避免同一档位连写两次。 */
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { className: effort_module_css_default.glow }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									children: level.name
								}, level.id) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									...
									disabled: !usable,
									"aria-label": "推理等级",
									"aria-valuetext": level !== void 0 ? level.name : "",
						...
						!usable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: disabled ? directoryError !== void 0 ? `模型目录加载失败：${directoryError}` : "模型目录加载中…" : "当前模型不提供多档推理等级"
						})
					]
				})]
			});
		...
		//#region apply
		/**
		 * 需要的客户端服务：connection + sessions（模型/会话）、theme（跟随官方主题配色）。
		 * 面板配色恒定跟随官方外观，不再自持外观设置（避免重复设置）。
		 */
		const inject = ["connection", "sessions", "theme"];
		/**
		 * 挂载浮动滑块面板；面板配色恒定跟随官方主题（theme.getTheme().active.colorScheme），
		 * 并在官方外观切换（theme/change）时实时重渲。
		...
		function apply(ctx) {
			...
			/** 面板配色 = 官方主题解析后的明暗（system 已被官方解析为 dark/light）。 */
			const currentThemeMode = () => resolveThemeMode(theme.getTheme());
			// -- 浮动滑块面板（拦截「推理等级」行）
			...
			const onDocClick = (event) => {
				...
				if (row instanceof HTMLElement) {
					// 优先读结构化的行标签（官方 cellLabel），兜底读整行文本；
					// 关键词覆盖当前界面语言：中文「推理等级」/ 英文「Effort」。
					const label = row.querySelector("[class*=\"cellLabel\"]")?.textContent ?? row.textContent ?? "";
					const text = label.trim();
					const isEffortRow = text.startsWith("推理等级") || text.startsWith("推理")
						|| text === "Effort" || text.startsWith("Effort");
					if (isEffortRow) {
			...
			ctx.effect(() => () => {
				...
				hidePanel();
				host.remove();
			}, "dsh-effort-slider: effort panel");
		}
		//#endregion
		exports.apply = apply;
